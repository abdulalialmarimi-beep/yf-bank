...
FUNNY_MESSAGES = {
    ...
    "mine_win":   ["💣 نجيت! يونان فاملي تنفس بعدين!", "يا بطل! حذرت الألغام!", "عبرت الحقل سالم! ما تستاهل شي أقل!"],
    "mine_lose":  ["💥 انفجرت! يونان فاملي تبعت لك الرحمة!", "ذهبت فلوسك مع الهواء يا لغيم!", "حتى الألغام ما تحبك!"],
    "rocket_win": ["🚀 طلعت بالوقت المناسب! يونان فاملي تفتخر!", "ذكي! تعرف متى تفر!", "مهندس الفرار الأول في يونان!"],
    "rocket_lose":["💥 انفجر الصاروخ وأنت فيه! يونان فاملي تقرأ عليك!", "طمعت وخسرت! الطمع شيطان يا بطل!", "الصاروخ قالك باي باي مع فلوسك!"],
    "roulette_win":["🎡 المال جاك من البيض! يونان فاملي تحسدك!", "الحظ معك اليوم! استغله قبل ما يروح!"],
    "roulette_lose":["🎡 الكرة ما تحبك! يونان فاملي تواسيك بالدعاء!", "حتى الروليت تعرف إنك مو محظوظ!"],
    "dragon_win": ["🐉 التنين يطيع يونان فاملي!", "خمنت صح! التنين يبوسك يا بطل!"],
    "dragon_lose":["🐉 التنين أكل فلوسك! يونان فاملي تقول لك عادي!", "التنين أذكى منك اليوم يا صديقي!"],
}
...
GAMES = {
    ...
    "mines":     {"name": "ألغام",    "icon": "💣", "desc": "تجنب الألغام واكسب!",       "min": 100, "max": 20000, "color": C_RED,    "image": "slots"},
    "rocket":    {"name": "صاروخ",   "icon": "🚀", "desc": "اطلع قبل الانفجار!",        "min": 100, "max": 50000, "color": C_ORANGE, "image": "crash"},
    "roulette":  {"name": "روليت",   "icon": "🎡", "desc": "أحمر أسود أو أرقام x35",   "min": 100, "max": 30000, "color": C_RED,    "image": "wheel"},
    "dragon":    {"name": "تنين",    "icon": "🐉", "desc": "يمين أو يسار؟",             "min": 100, "max": 25000, "color": C_PURPLE, "image": "slots"},
}
...
# ─── ألغام ───────────────────────────────────────────────────────
@bot.command(name="ألغام")
async def mines_cmd(ctx, bet: int):
    data = get_user(bot.db, str(ctx.author.id))
    if bet < 100 or bet > 20000:
        await ctx.send("❌ الرهان: $100 - $20,000!"); return
    if data["balance"] < bet:
        await ctx.send(embed=emb("❌ فلوسك ما تكفي!", get_funny("broke"), C_RED)); return
    ...
    GRID = 9
    mines_count = 3
    mine_positions = set(random.sample(range(GRID), mines_count))
    revealed = set()
    multipliers = [1.3, 1.7, 2.2, 3.0, 4.0, 5.5, 8.0, 12.0, 20.0]
    current_mult = 1.0
    cashed_out = [False]
    exploded = [False]
    ...
    def make_grid():
        ...
        for i in range(0, GRID, 3):
            ...
            for j in range(3):
                pos = i + j
                if pos in revealed:
                    row += "💰 " if pos not in mine_positions else "💥 "
                else:
                    row += "⬛ "
            rows.append(row)
        safe = GRID - mines_count - len(revealed)
        mult = multipliers[min(len(revealed), len(multipliers)-1)]
        return "\n".join(rows) + f"\n\n💎 الربح الحالي: **${int(bet * mult):,}** (x{mult})\n🟩 خلايا آمنة: {safe} | 💣 ألغام: {mines_count}"

    view = discord.ui.View(timeout=60)

    async def make_btn(pos):
        async def callback(i: discord.Interaction):
            if i.user.id != ctx.author.id:
                await i.response.send_message("مو لعبتك!", ephemeral=True); return
            if cashed_out[0] or exploded[0]:
            ...
            if pos in mine_positions:
                ...
                new_ach = check_achievements(data)
                save_db(bot.db)
                e = emb("💥 " + get_funny("mine_lose"), f"ضربت لغم! خسرت **${bet:,}**\n💣 اللغم كان في الخلية {pos+1}!", C_RED)
                e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
                add_ach_field(e, new_ach)
                for child in view.children:
                    child.disabled = True
                await i.response.edit_message(content=make_grid(), embed=e, view=view)
            else:
                nonlocal current_mult
                current_mult = multipliers[min(len(revealed)-1, len(multipliers)-1)]
                if len(revealed) >= GRID - mines_count:
                    cashed_out[0] = True
                    winnings = int(bet * multipliers[-1])
                    data["balance"] += winnings
                    data["total_earned"] += winnings - bet
                    data["wins"] += 1
                    data["games_played"] += 1
                    ...
                    new_ach = check_achievements(data)
                    save_db(bot.db)
                    e = emb("🏆 كشفت كل الخلايا!", f"ربحت **${winnings:,}**! x{multipliers[-1]}", C_GREEN)
                    e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
                    add_ach_field(e, new_ach)
                    for child in view.children:
                        child.disabled = True
                    ...
                    await i.response.edit_message(content=make_grid(), view=view)

        return callback

    async def cashout_cb(i: discord.Interaction):
        if i.user.id != ctx.author.id:
            await i.response.send_message("مو لعبتك!", ephemeral=True); return
        if cashed_out[0] or exploded[0] or len(revealed) == 0:
            await i.response.defer(); return
        cashed_out[0] = True
        winnings = int(bet * current_mult)
        data["balance"] += winnings
        data["total_earned"] += winnings - bet
        data["wins"] += 1
        data["games_played"] += 1
        ...
        new_ach = check_achievements(data)
        save_db(bot.db)
        e = emb("💰 " + get_funny("mine_win"), f"سحبت **${winnings:,}**! (x{current_mult})", C_GREEN)
        e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
        add_ach_field(e, new_ach)
        for child in view.children:
            child.disabled = True
    ...
    for pos in range(GRID):
        btn = discord.ui.Button(label=f"⬛", style=discord.ButtonStyle.secondary, row=pos//3)
        btn.callback = await make_btn(pos)
        view.add_item(btn)

    cashout_btn = discord.ui.Button(label="💰 اسحب", style=discord.ButtonStyle.success, row=3)
    cashout_btn.callback = cashout_cb
    view.add_item(cashout_btn)

    await ctx.send(content=make_grid(), view=view)


# ─── صاروخ ───────────────────────────────────────────────────────
@bot.command(name="صاروخ")
async def rocket_cmd(ctx, bet: int):
    data = get_user(bot.db, str(ctx.author.id))
    if bet < 100 or bet > 50000:
        await ctx.send("❌ الرهان: $100 - $50,000!"); return
    if data["balance"] < bet:
    ...
    crash_point = round(random.choices(
        [1.0, 1.2, 1.5, 2.0, 3.0, 5.0, 10.0, 25.0, 50.0, 100.0],
        weights=[20, 20, 18, 15, 10, 7, 5, 3, 1.5, 0.5]
    )[0], 2)
    ...
    def rocket_display():
        bar_len = 15
        progress = min(int((current_mult[0] - 1) / (crash_point - 1) * bar_len), bar_len) if crash_point > 1 else bar_len
        bar = "🟩" * progress + "⬜" * (bar_len - progress)
        return f"🚀 الصاروخ يطلع!\n\n{bar}\n\n📈 المضاعف: **x{current_mult[0]:.2f}**\n💰 الربح الحالي: **${int(bet * current_mult[0]):,}**\n💣 نقطة الانفجار: ???"

    view = discord.ui.View(timeout=30)
    cashout_btn = discord.ui.Button(label="🪂 انزل الحين!", style=discord.ButtonStyle.success)

    async def cashout_cb(i: discord.Interaction):
        if i.user.id != ctx.author.id:
            await i.response.send_message("مو لعبتك!", ephemeral=True); return
        if cashed_out[0] or crashed[0]:
            await i.response.defer(); return
        cashed_out[0] = True
        winnings = int(bet * current_mult[0])
        data["balance"] += winnings
        data["total_earned"] += winnings - bet
        data["wins"] += 1
        data["games_played"] += 1
        ...
        e = emb("🪂 " + get_funny("rocket_win"), f"نزلت بـ **x{current_mult[0]:.2f}** وربحت **${winnings:,}**!\n💣 الصاروخ انفجر عند x{crash_point}", C_GREEN)
        e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
        add_ach_field(e, new_ach)
        await i.response.edit_message(content=None, embed=e, view=view)

    cashout_btn.callback = cashout_cb
    view.add_item(cashout_btn)

    msg = await ctx.send(content=rocket_display(), view=view)

    step = 0.15
    while current_mult[0] < crash_point and not cashed_out[0]:
        await asyncio.sleep(1.2)
        if cashed_out[0]: break
        current_mult[0] = round(current_mult[0] + step + random.uniform(0, step), 2)
        step = round(step * 1.08, 3)
        if not cashed_out[0]:
            try:
                await msg.edit(content=rocket_display())
            except:
                pass
    ...
    if not cashed_out[0]:
        ...
        new_ach = check_achievements(data)
        save_db(bot.db)
        e = emb("💥 " + get_funny("rocket_lose"), f"الصاروخ انفجر عند **x{crash_point}**!\nخسرت **${bet:,}**", C_RED)
        e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
        add_ach_field(e, new_ach)
        try:
            await msg.edit(content=None, embed=e, view=view)
        except:
            pass
...
async def roulette_cmd(ctx, bet: int):
    data = get_user(bot.db, str(ctx.author.id))
    if bet < 100 or bet > 30000:
        await ctx.send("❌ الرهان: $100 - $30,000!"); return
    if data["balance"] < bet:
        await ctx.send(embed=emb("❌ فلوسك ما تكفي!", get_funny("broke"), C_RED)); return
    ...
    async def make_choice(choice_type, mult, label):
        async def cb(i: discord.Interaction):
            if i.user.id != ctx.author.id:
                await i.response.send_message("مو لعبتك!", ephemeral=True); return
            chosen[0] = (choice_type, mult, label)
            for child in view.children:
                child.disabled = True

            spin_frames = ["🎡 تدور...", "🔴⚫🟢 تدور...", "⚫🟢🔴 تدور...", "🟢🔴⚫ تدور..."]
            await i.response.edit_message(content=spin_frames[0], view=view)
            for frame in spin_frames[1:]:
                await asyncio.sleep(0.7)
                await i.message.edit(content=frame)
            ...
            win = False
            if choice_type == "red" and result in reds: win = True
            elif choice_type == "black" and result not in reds and result != 0: win = True
            elif choice_type == "green" and result == 0: win = True
            elif choice_type == "number" and result == mult: win = True

            actual_mult = {"red": 2, "black": 2, "green": 14, "number": 35}.get(choice_type, 2)

            if win:
                winnings = bet * actual_mult
                data["balance"] += winnings - bet
                data["total_earned"] += winnings - bet
                data["wins"] += 1
                color = C_GREEN
                title = get_funny("roulette_win")
                result_text = f"فزت بـ **${winnings:,}**! (x{actual_mult})"
            else:
                data["balance"] -= bet
                data["total_lost"] += bet
                data["losses"] += 1
                color = C_RED
                title = get_funny("roulette_lose")
                result_text = f"خسرت **${bet:,}**!"
            ...
            e.add_field(name="🎡 النتيجة", value=f"{result_color} **{result}** — {result_name}", inline=False)
            e.add_field(name="🎯 اختيارك", value=label, inline=True)
            e.add_field(name="💰 النتيجة", value=result_text, inline=True)
            e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}", inline=False)
            add_ach_field(e, new_ach)
            await i.message.edit(content=None, embed=e)

        return cb

    red_btn   = discord.ui.Button(label="🔴 أحمر x2",  style=discord.ButtonStyle.danger,   row=0)
    black_btn = discord.ui.Button(label="⚫ أسود x2",  style=discord.ButtonStyle.secondary, row=0)
    green_btn = discord.ui.Button(label="🟢 صفر x14",  style=discord.ButtonStyle.success,   row=0)

    red_btn.callback   = await make_choice("red",   2,  "🔴 أحمر")
    black_btn.callback = await make_choice("black", 2,  "⚫ أسود")
    green_btn.callback = await make_choice("green", 14, "🟢 صفر")

    view.add_item(red_btn)
    view.add_item(black_btn)
    view.add_item(green_btn)

    # Add random number buttons
    for _ in range(3):
        num = random.randint(1, 36)
        num_btn = discord.ui.Button(label=f"#{num} x35", style=discord.ButtonStyle.primary, row=1)
        num_btn.callback = await make_choice("number", num, f"#{num}")
        view.add_item(num_btn)

    e = emb("🎡 روليت يونان فاملي!", f"رهانك: **${bet:,}**\nاختار لونك أو رقمك!", C_GOLD)
...
async def dragon_cmd(ctx, bet: int):
    ...
    if bet < 100 or bet > 25000:
        await ctx.send("❌ الرهان: $100 - $25,000!"); return
    if data["balance"] < bet:
        await ctx.send(embed=emb("❌ فلوسك ما تكفي!", get_funny("broke"), C_RED)); return
    ...
    mults = [1.9, 3.5, 6.5, 12.0, 22.0, 40.0, 75.0]
    round_num = [0]
    ...
    def game_display():
        stars = "⭐" * round_num[0]
        return f"🐉 **لعبة التنين**\n\n{stars}\n\n🎯 الجولة: **{round_num[0] + 1}**\n💰 الربح المحتمل: **${int(current_winnings[0] * mults[min(round_num[0], len(mults)-1)]):,}**\n🏆 إذا فزت الكل: **x{mults[min(round_num[0], len(mults)-1)]}**"

    view = discord.ui.View(timeout=45)

    async def choose(direction: str, i: discord.Interaction):
        if i.user.id != ctx.author.id:
            await i.response.send_message("مو لعبتك!", ephemeral=True); return
        if game_over[0]:
            await i.response.defer(); return

        correct = random.choice(["يمين", "يسار"])
        if direction == correct:
            round_num[0] += 1
            if round_num[0] >= len(mults):
                game_over[0] = True
                winnings = int(bet * mults[-1])
                data["balance"] += winnings
                data["total_earned"] += winnings - bet
                data["wins"] += 1
                data["games_played"] += 1
                ...
                for child in view.children:
                    child.disabled = True
                e = emb("🏆 " + get_funny("dragon_win"), f"فزت بكل الجولات! **${winnings:,}** (x{mults[-1]})", C_GOLD)
                e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
                add_ach_field(e, new_ach)
                await i.response.edit_message(content=None, embed=e, view=view)
            else:
                await i.response.edit_message(content=game_display(), view=view)
        else:
            game_over[0] = True
            data["total_lost"] += bet
            data["losses"] += 1
            ...
            for child in view.children:
                child.disabled = True
            e = emb("💥 " + get_funny("dragon_lose"), f"التنين كان **{correct}**! خسرت **${bet:,}**", C_RED)
            e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
            add_ach_field(e, new_ach)
            await i.response.edit_message(content=None, embed=e, view=view)
    ...
    async def cashout_cb(i: discord.Interaction):
        if i.user.id != ctx.author.id:
            await i.response.send_message("مو لعبتك!", ephemeral=True); return
        if game_over[0] or round_num[0] == 0:
            await i.response.defer(); return
        game_over[0] = True
        mult = mults[round_num[0] - 1]
        winnings = int(bet * mult)
        data["balance"] += winnings
        data["total_earned"] += winnings - bet
        data["wins"] += 1
        ...
        for child in view.children:
            child.disabled = True
        e = emb("💰 سحبت!", f"ربحت **${winnings:,}** (x{mult}) بعد {round_num[0]} جولات!", C_GREEN)
        e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
        add_ach_field(e, new_ach)
        await i.response.edit_message(content=None, embed=e, view=view)

    right_btn   = discord.ui.Button(label="👉 يمين", style=discord.ButtonStyle.primary,  row=0)
    left_btn    = discord.ui.Button(label="👈 يسار", style=discord.ButtonStyle.primary,  row=0)
    cashout_btn = discord.ui.Button(label="💰 اسحب", style=discord.ButtonStyle.success,  row=1)

    async def right_cb(i): await choose("يمين", i)
    async def left_cb(i):  await choose("يسار", i)
    ...
    await ctx.send(content=game_display(), view=view)
...
async def start_cmd(ctx):
    ...
    embed.add_field(name="🎮 الألعاب",       value="سلات، عجلة، نرد، دجاجة، فواكه، ألوان، صناديق، قمار، بلاكجاك، تكس، حظ، ألغام، صاروخ، روليت، تنين", inline=False)
