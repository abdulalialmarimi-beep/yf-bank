...
FUNNY = {
    ...
    "marry_bot":  ["🤖 البوت ما يتزوج! تبي تتزوج كود Python؟"],
...
async def marry_cmd(ctx, user: discord.User):
    ...
    if data["married_to"]:
        await ctx.send(embed=emb("😂 انت متزوج!", funny("already_married"), C_RED)); return
    ...
    async def no_cb(i: discord.Interaction):
        if i.user.id != user.id:
            await i.response.send_message("❌ مو لك!", ephemeral=True); return
        e2 = emb("💔 رُفض!", f"{user.mention} رفض {ctx.author.mention}! 😂", C_RED)
        await i.response.edit_message(embed=e2, view=None)
...
async def divorce_cmd(ctx):
    ...
    partner = get_user(bot.db, data["married_to"])
    partner["married_to"] = None
    data["married_to"]    = None
    data["divorce_count"] = data.get("divorce_count", 0) + 1
    save_db(bot.db)
    await ctx.send(embed=emb("💔 طلاق!", "تم الطلاق! يونان فاملي حزينة عليكم 😢", C_RED))
...
# ═══════════════════════════════════════════════════════════════════
# أوامر نصية للسوق والممتلكات (بديل لمن يفضل النص)
# ═══════════════════════════════════════════════════════════════════
