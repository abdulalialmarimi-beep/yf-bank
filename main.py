...
class MarketMenuView(discord.ui.View):
    def __init__(self, ctx):
        ...
        self.select = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(label=v["name"], value=k, emoji=v["icon"][:1] if len(v["icon"]) == 1 else None)
...
async def marry_cmd(ctx, user: discord.User):
    ...
    # الزوج يقدر يتزوج ثانية
    is_second = data["married_to"] is not None

    if is_second:
        e = emb("💍 زواج ثاني؟!", f"👀 {ctx.author.mention} يتجرأ ويعرض الزواج على {user.mention}!\nالمهر: **$5,000** 💎\n\n⚠️ زوجتك الأولى بتعرف! 🌝", C_PINK)
    else:
        e = emb("💍 عرض زواج!", f"{ctx.author.mention} يعرض الزواج على {user.mention}!\nالمهر: **$5,000** 💎", C_PINK)
    ...
    async def yes_cb(i: discord.Interaction):
        ...
        # إذا زواج ثاني — احفظ الزوجة الأولى وأبلّغها
    ...
    async def no_cb(i: discord.Interaction):
        if i.user.id != user.id:
            await i.response.send_message("❌ مو لك!", ephemeral=True); return
        e2 = emb("💔 رُفض!", f"{user.mention} رفض/ت {ctx.author.mention}! 😂", C_RED)
        await i.response.edit_message(embed=e2, view=None)
...
async def divorce_cmd(ctx):
    ...
    # تحقق إذا الزوجة تحاول تطلق (الزوجة ما تقدر تطلق)
    if data.get("role") == "wife":
    ...
    try:
        ...
        is_bot_partner = partner_user.bot
    except:
        is_bot_partner = False
    ...
    if is_bot_partner:
        # طلاق البوت
        await ctx.send(embed=emb("💔 طلّقت البوت؟!", funny("marry_bot_bot"), C_RED))
    else:
        # طلاق عضو عادي
...
async def protection_cmd(ctx, hours: int = 24):
    ...
    data["protection_expires"] = (datetime.now() + timedelta(hours=hours)).isoformat()
    save_db(bot.db)
    e = emb("🛡️ محمي!", f"{funny('protection_active')}\nمحمي لمدة **{hours} ساعة** بـ **${cost:,}**", C_BLUE)
    e.add_field(name="🪙 رصيدك", value=f"${data['balance']:,}")
    await ctx.send(embed=e)
