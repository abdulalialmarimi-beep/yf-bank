...
COMMAND_PREFIX = "yf"
C_GOLD = 0xd4a843; C_GREEN = 0x4ade80; C_RED = 0xf87171; C_BLUE = 0x60a5fa; C_PINK = 0xff69b4; C_PURPLE = 0x9333ea; C_ORANGE = 0xf97316
...
def get_user(db, uid: str):
    if uid not in db:
        db[uid] = {
            "balance": 5000, "bank": 0, "level": 1, "xp": 0, "streak": 0, "last_daily": None,
            "total_earned": 0, "total_lost": 0, "total_gambled": 0,
            "properties": {"cars": 0, "stocks": 0, "lands": 0, "trains": 0, "phones": 0, "companies": 0, "servers": 0, "diamonds": 0},
            "inventory": {}, "married_to": None, "married_since": None, "dowry": 0,
            "divorce_count": 0, "last_divorce": None, "protection": None, "protection_expires": None,
...
class YFBank(commands.Bot):
    ...
    async def income_collector(self):
        for uid, data in self.db.items():
            total_income = sum(data["properties"].get(k, 0) * prop["income"] for k, prop in PROPERTIES.items())
            if total_income > 0:
...
async def balance_cmd(interaction: discord.Interaction, user: Optional[discord.User] = None):
    ...
    embed.add_field(name=f"⚡ المستوى {data['level']} — {get_rank(data['level'])}", value=f"`{bar}` {xp_progress/xp_needed*100:.1f}%", inline=False)
...
async def top_cmd(interaction: discord.Interaction):
    ...
    users_data = [(m, d["balance"]+d["bank"], d["level"]) for uid, d in bot.db.items() if (m:=interaction.guild.get_member(int(uid)))]
    users_data.sort(key=lambda x: x[1], reverse=True)
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
    for i, (member, total, lvl) in enumerate(users_data[:10]):
        embed.add_field(name=f"{medals[i] if i<3 else f'#{i+1}'} {member.display_name}", value=f"💰 ${format_money(total)} | ⚡ Lvl {lvl}", inline=False)
    view = discord.ui.View()
...
async def market_cmd(interaction: discord.Interaction):
    ...
    for key, item in bot.market.items():
        ...
        embed.add_field(name=f"{item['icon']} {item['name']}", value=f"{emoji} شراء: ${format_money(item['buy'])}\nبيع: ${format_money(item['sell'])}\nتغير: {change:+.2f}%", inline=True)
    view = discord.ui.View()
...
async def games_cmd(interaction: discord.Interaction):
    ...
    for key, game in GAMES.items():
        embed.add_field(name=f"{game['icon']} {game['name']}", value=f"{game['desc']}\nحد: ${game['min']:,}-${game['max']:,}", inline=True)
    select = discord.ui.Select(placeholder="🎮 اختر لعبة...", options=[discord.SelectOption(label=g["name"], description=f"{g['desc']} | ${g['min']:,}-${g['max']:,}", value=k, emoji=g["icon"]) for k, g in GAMES.items()])
...
async def wheel_cmd(interaction: discord.Interaction, bet: int):
    ...
    if bet < 100 or bet > 50000:
        await interaction.response.send_message("❌ الرهان: $100 - $50,000!", ephemeral=True); return
    if data["balance"] < bet:
        await interaction.response.send_message("❌ رصيدك لا يكفي!", ephemeral=True); return
    segments = [("0x 💀", 0.0, "💀", C_RED), ("0.5x 🔴", 0.5, "🔴", C_RED), ("0.8x ⚫", 0.8, "⚫", C_ORANGE),
                ("1x 🔵", 1.0, "🔵", C_BLUE), ("1.5x 🟢", 1.5, "🟢", C_GREEN), ("2x 💚", 2.0, "💚", C_GREEN),
                ("3x 🌟", 3.0, "🌟", C_GOLD), ("5x ✨", 5.0, "✨", C_GOLD), ("10x 💎", 10.0, "💎", C_PURPLE)]
    weights = [8, 12, 15, 20, 18, 12, 8, 5, 2]
    await interaction.response.defer()
    msg = await interaction.followup.send("🎡 تدور العجلة...")
    for _ in range(5):
    ...
    if mult > 1:
        ...
        text = f"😊 استردت **${winnings:,}**"; color = C_BLUE
    elif mult > 0:
        data["balance"] -= (bet - winnings); data["total_lost"] += (
